...
func check(ck *Clerk, key string, value string) {
	actual := ck.Get(key)
	if actual != value {
		log.Fatalf("Get(%v) -> %v, expected %v", key, actual, value)
	}
}
...
func proxy(t *testing.T, port string, delay *atomic.Uint32) {
	portx := port + "x"
	os.Remove(portx)
	...
	go func() {
		...
		for {
			...
			time.Sleep(time.Duration(delay.Load()) * time.Second)
...
func TestPartition1(t *testing.T) {
	...
	// longer the primary.
	delay.Store(uint32(4))
	staleGet := atomic.Bool{}
	go func() {
		x := ck1.Get("a")
		if x == "1" {
			staleGet.Store(true)
